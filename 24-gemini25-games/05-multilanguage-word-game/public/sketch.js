// @ts-check
// Welcome to the Multilingual Anagram Fun Zone! 🥳 v1.5.0
// Changes: Implemented tap-select/tap-target interaction for Touch input.
//          Removed touch drag logic. Mouse/Keyboard remain drag/select+swap.
//          Version in title.
...
const GAME_VERSION = "1.5.0"; // Updated version number
...
const COLORS = {
    text: '#333333',
    letterBg: '#ffffff',
    letterBgSelected: '#ffff99',          // Keyboard Pair Selection / Mouse Dragging
    letterBgSelectedKeyboard: '#add8e6', // Keyboard Cursor Highlight
    letterBgTouchSelectedSource: '#d3d3d3', // Light Gray for Touch Source Selection *** NEW ***
    letterBgTouchTarget: '#ffb6c1',       // Light Pink for Touch Target Highlight *** NEW ***
    letterBorder: '#cccccc',
...
// --- lib/utils.js ---
// (shuffleArray, scrambleWord, isPointInRect, getFlagEmoji remain the same as v1.4.1)
...
const words = [ /* ... same word data as before ... */
    // Easy
    { emoji: '🍎', it: 'mela', en: 'apple', de: 'apfel', fr: 'pomme', difficulty: 'easy' }, { emoji: '👀', it: 'occhi', en: 'eyes', de: 'augen', fr: 'yeux', difficulty: 'easy' }, { emoji: '🏠', it: 'casa', en: 'house', de: 'haus', fr: 'maison', difficulty: 'easy' }, { emoji: '🐶', it: 'cane', en: 'dog', de: 'hund', fr: 'chien', difficulty: 'easy' }, { emoji: '🐱', it: 'gatto', en: 'cat', de: 'katze', fr: 'chat', difficulty: 'easy' }, { emoji: '☀️', it: 'sole', en: 'sun', de: 'sonne', fr: 'soleil', difficulty: 'easy' }, { emoji: '🌙', it: 'luna', en: 'moon', de: 'mond', fr: 'lune', difficulty: 'easy' }, { emoji: '💧', it: 'acqua', en: 'water', de: 'wasser', fr: 'eau', difficulty: 'easy' }, { emoji: '🌳', it: 'albero', en: 'tree', de: 'baum', fr: 'arbre', difficulty: 'easy' }, { emoji: '🚗', it: 'auto', en: 'car', de: 'auto', fr: 'voiture', difficulty: 'easy' },
    // Medium
    { emoji: '🍓', it: 'fragola', en: 'strawberry', de: 'erdbeere', fr: 'fraise', difficulty: 'medium' }, { emoji: '🐘', it: 'elefante', en: 'elephant', de: 'elefant', fr: 'éléphant', difficulty: 'medium' }, { emoji: '⏰', it: 'orologio', en: 'clock', de: 'uhr', fr: 'horloge', difficulty: 'medium' }, { emoji: '🦋', it: 'farfalla', en: 'butterfly', de: 'schmetterling', fr: 'papillon', difficulty: 'medium' }, { emoji: '🌻', it: 'girasole', en: 'sunflower', de: 'sonnenblume', fr: 'tournesol', difficulty: 'medium' }, { emoji: '🌍', it: 'mondo', en: 'world', de: 'welt', fr: 'monde', difficulty: 'medium' }, { emoji: '🎤', it: 'microfono', en: 'microphone', de: 'mikrofon', fr: 'microphone', difficulty: 'medium' },
    // Hard
    { emoji: '♊️', it: 'gemelli', en: 'twins', de: 'zwillinge', fr: 'jumeaux', difficulty: 'hard' }, { emoji: '🧭', it: 'bussola', en: 'compass', de: 'kompass', fr: 'boussole', difficulty: 'hard' }, { emoji: '🔬', it: 'microscopio', en: 'microscope', de: 'mikroskop', fr: 'microscope', difficulty: 'hard' }, { emoji: '🤔', it: 'pensare', en: 'thinking', de: 'denken', fr: 'penser', difficulty: 'hard' }, { emoji: '🚀', it: 'razzo', en: 'rocket', de: 'rakete', fr: 'fusée', difficulty: 'hard' }, { emoji: '🥳', it: 'festa', en: 'party', de: 'party', fr: 'fête', difficulty: 'hard' },
];
...
// Mouse Dragging State (only for mouse input now)
...
function drawStartScreen(p) {
    // (Same as v1.4.1 - Version already in title)
    ...
    let hoverX = p.mouseX; let hoverY = p.mouseY; // Check hover normally for mouse
    p.fill(COLORS.buttonBg); if (isPointInRect(hoverX, hoverY, startBtnX, startBtnY, startBtnW, startBtnH)) { p.fill(COLORS.buttonHover); } p.rect(startBtnX, startBtnY, startBtnW, startBtnH, 10);
...
// *** MODIFIED: Added touch selection highlighting ***
function drawPlayingScreen(p) {
    ...
    wordStates.forEach((langState, langIndex) => {
        ...
        langState.currentLetters.forEach((letter, letterIndex) => {
            langState.letterBoxes.push({ x: currentLetterX, y: rowStartY, w: letterSize, h: letterSize });
            let bgColor = COLORS.letterBg; // Default

            // --- Highlighting Logic (Touch takes precedence visually) ---
            const isTouchSelectedSource = touchSelectionState.active && touchSelectionState.langIndex === langIndex && touchSelectionState.letterIndex === letterIndex;
            const isTouchTarget = touchSelectionState.active && touchSelectionState.langIndex === langIndex && touchSelectionState.letterIndex !== letterIndex;
            const isKeyboardSelectedPair = keyboardState.selectedLetters.some(sel => sel.langIndex === langIndex && sel.letterIndex === letterIndex);
            const isKeyboardCursor = keyboardState.selectedRow === langIndex && keyboardState.selectedCol === letterIndex;

            if (isTouchSelectedSource) {
                bgColor = COLORS.letterBgTouchSelectedSource; // Gray
            } else if (isTouchTarget) {
                bgColor = COLORS.letterBgTouchTarget; // Pink
            } else if (isKeyboardSelectedPair) {
                bgColor = COLORS.letterBgSelected; // Yellow (Keyboard swap pair)
            } else if (isKeyboardCursor) {
                bgColor = COLORS.letterBgSelectedKeyboard; // Blue (Keyboard cursor)
            }
     ...
     // Draw mouse dragging letter (touch doesn't drag visually anymore)
     if (dragging.active && !touchSelectionState.active) { // Only draw mouse drag if touch isn't selecting
         const { letterIndex, langIndex, offsetX, offsetY } = dragging; const dragX = dragging.currentX - offsetX; const dragY = dragging.currentY - offsetY; const letter = wordStates[langIndex]?.currentLetters[letterIndex];
         if (letter) { p.stroke(COLORS.letterBorder); p.fill(COLORS.letterBgSelected); p.rect(dragX, dragY, letterSize, letterSize, 5); applyTextStyle(p, { size: letterFontSize, align: p.CENTER, vAlign: p.CENTER }); p.text(letter.toUpperCase(), dragX + letterSize / 2, dragY + letterSize / 2); }
     }
}
...
// --- MOUSE Handlers --- (No changes needed for mouse logic)
function handleMousePressed(p) { if (touchSelectionState.active) return; /* Prevent mouse action if touch is selecting */ if (dragging.active) return; let pressX = p.mouseX; let pressY = p.mouseY; if (gameState === 'START_SCREEN') { handleStartScreenInput(p, pressX, pressY); } else if (gameState === 'PLAYING') { if (!handlePlayingScreenButtons(p, pressX, pressY)) { startDragging(p, pressX, pressY); } } else if (gameState === 'GAME_OVER') { handleGameOverInput(p, pressX, pressY); } }
function handleMouseDragged(p) { if (dragging.active) { dragging.currentX = p.mouseX; dragging.currentY = p.mouseY; } return false; } // Prevent default only if dragging
function handleMouseReleased(p) { if (dragging.active) { finishDragging(p, p.mouseX, p.mouseY); } }

// --- TOUCH Handler --- (*** NEW Logic using touchSelectionState ***)
function handleTouchStarted(p) {
    // Prevent action if mouse is already dragging (edge case, unlikely)
    if (dragging.active) return false;
    // Ignore multi-touch after the first one for simplicity
    if (p.touches.length > 1 && !touchSelectionState.active) {
         console.log("Ignoring multi-touch start.");
         return false;
    }

    // Use the first touch point for interaction
    // Note: p.touches contains current touches; use it directly
    let pressX = p.mouseX; // p5 translates first touch to mouseX/Y
    let pressY = p.mouseY;

    if (gameState === 'START_SCREEN') {
        handleStartScreenInput(p, pressX, pressY);
    } else if (gameState === 'PLAYING') {
        // Prioritize Button Check
        if (handlePlayingScreenButtons(p, pressX, pressY)) {
            resetTouchSelectionState(); // Cancel selection if button hit
            return false; // Button handled, prevent letter selection
        }

        // --- Tap-Select / Tap-Target Logic ---
        ...
        let targetHit = null; // { langIndex, letterIndex }

        // Find which letter (if any) was tapped
        for (let i = 0; i < wordStates.length; i++) {
            if (!wordStates[i] || !wordStates[i].letterBoxes) continue;
            for (let j = 0; j < wordStates[i].letterBoxes.length; j++) {
                const box = wordStates[i].letterBoxes[j];
                if (isPointInRect(pressX, pressY, box.x, box.y, letterSize, letterSize)) {
                    targetHit = { langIndex: i, letterIndex: j };
                    break;
                }
            }
            if (targetHit) break;
        ...
        if (!touchSelectionState.active) {
            ...
            if (targetHit) {
                ...
                touchSelectionState.langIndex = targetHit.langIndex;
                touchSelectionState.letterIndex = targetHit.letterIndex;
                console.log(`Touch selected SOURCE: [${targetHit.langIndex}, ${targetHit.letterIndex}]`);
                clearKeyboardSelection(); // Clear keyboard selection when touch starts
            } else {
                 // Tap was not on a letter or button, do nothing
                 console.log("Touch tap missed letters/buttons.");
            }
        } else {
            // --- Second Tap: Select Target (or Cancel) ---
            if (targetHit &&
                targetHit.langIndex === touchSelectionState.langIndex && // Must be same language/row
                targetHit.letterIndex !== touchSelectionState.letterIndex) // Must be different letter
            {
                // Valid Target Tapped - Perform Swap
                console.log(`Touch selected TARGET: [${targetHit.langIndex}, ${targetHit.letterIndex}]. Swapping with source [${touchSelectionState.langIndex}, ${touchSelectionState.letterIndex}].`);
                swapLetters(touchSelectionState.langIndex, touchSelectionState.letterIndex, targetHit.letterIndex);
                checkWinCondition(p);
                resetTouchSelectionState(); // Reset after successful swap
            } else {
                // Invalid Target Tapped (different row, same letter, or empty space) - Cancel Selection
                 if (targetHit) {
                      console.log(`Touch tap on invalid target [${targetHit.langIndex}, ${targetHit.letterIndex}]. Cancelling selection.`);
                 } else {
                      console.log("Touch tap missed target letters. Cancelling selection.");
                 }
                resetTouchSelectionState();
            }
        }
    } else if (gameState === 'GAME_OVER') {
        handleGameOverInput(p, pressX, pressY);
    }

    return false; // Prevent default browser actions like double-tap zoom etc.
}

// --- REMOVED handleTouchMoved and handleTouchEnded ---
// They are not needed for the tap-tap interaction model.
...
function handlePlayingScreenButtons(p, pressX, pressY) { const resetBtn = uiElements.resetButton; if (resetBtn && isPointInRect(pressX, pressY, resetBtn.x, resetBtn.y, resetBtn.w, resetBtn.h)) { console.log("Reset clicked!"); gameState = 'GAME_OVER'; resetDraggingState(); /* Reset mouse drag just in case */ return true; } return false; }
...
// --- Dragging Logic Helpers (Mouse Only) ---
function startDragging(p, pressX, pressY) { // Simplified: Removed touch params
     const headerHeight = p.height * HEADER_HEIGHT_PERCENT; const gameAreaHeight = p.height * GAME_AREA_HEIGHT_PERCENT; const numRows = wordStates.length > 0 ? wordStates.length : 1; const availableHeightPerRow = gameAreaHeight / numRows; const letterSize = p.constrain(availableHeightPerRow * 0.4, 25, DEFAULT_LETTER_SIZE);
     for (let i = 0; i < wordStates.length; i++) { if (!wordStates[i] || !wordStates[i].letterBoxes) continue; for (let j = 0; j < wordStates[i].letterBoxes.length; j++) { const box = wordStates[i].letterBoxes[j]; if (isPointInRect(pressX, pressY, box.x, box.y, letterSize, letterSize)) {
         dragging.active = true; // isTouch is always false here
         dragging.langIndex = i; dragging.letterIndex = j; dragging.offsetX = pressX - box.x; dragging.offsetY = pressY - box.y; dragging.currentX = pressX; dragging.currentY = pressY;
         clearTouchSelectionState(); // Cancel touch selection if mouse drag starts
         clearKeyboardSelection(); // Cancel keyboard selection too
         console.log(`Mouse Drag started: lang ${i}, letter ${j}.`);
         return;
     } } }
 }
function finishDragging(p, releaseX, releaseY) {
     if (!dragging.active) return;
     console.log(`Mouse Drag ended at (${releaseX.toFixed(0)}, ${releaseY.toFixed(0)})`);
...
// --- Touch & Keyboard State Reset Helpers ---
...
const sketch = (p) => {
    ...
    p.mouseReleased = () => handleMouseReleased(p);

    // Only use touchStarted for the tap interaction model
    p.touchStarted = () => handleTouchStarted(p);
    // *** REMOVE touchMoved and touchEnded assignments ***
    // p.touchMoved = () => handleTouchMoved(p);  // No longer needed
    // p.touchEnded = () => handleTouchEnded(p);  // No longer needed
