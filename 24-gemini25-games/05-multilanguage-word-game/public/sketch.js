// @ts-check
// Welcome to the Multilingual Anagram Fun Zone! 🥳 v1.3.1
// Fixes: Added missing return in shuffleArray
...
function scrambleWord(word) {
    ...
    // Avoid infinite loop for words with duplicate letters by shuffling first
    if (letters.length > 1) {
        letters = shuffleArray([...letters]); // Shuffle a copy first
        ...
        if (shuffledWord === word && letters.length > 2) {
             attempts = 0; // Reset attempts counter for the loop
            do {
                letters = shuffleArray(letters); // Re-shuffle the already shuffled array
                shuffledWord = letters.join('');
                attempts++;
            } while (shuffledWord === word && attempts < maxAttempts);
        }
         // Handle specific case: 2 identical letters (e.g., "oo") - shuffleArray might return original order
        if (letters.length === 2 && letters[0] === letters[1]) {
             shuffledWord = letters.join(''); // Ensure it's correctly joined
        }
    }
    return shuffledWord;
}
...
let wordsPlayed = 0;
let startTime = 0;
let correctFlashTime = 0;
let currentWordPool = [];

let selectedDifficulty = 'easy';
let availableLanguages = ['it', 'en', 'de', 'fr'];
let selectedLanguages = ['it', 'en', 'de'];

let dragging = {
    active: false, isTouch: false, pointerId: null,
    letterIndex: -1, langIndex: -1,
    offsetX: 0, offsetY: 0, currentX: 0, currentY: 0
};
...
function drawStartScreen(p) {
    // (Start screen drawing logic remains the same as v1.3)
    ...
    const toggleW = p.constrain(p.width / 6, 70, 90);
    const toggleH = 30;
    const toggleSpacing = 15;
    availableLanguages.forEach((lang, index) => {
        const flag = getFlagEmoji(lang);
        const isSelected = selectedLanguages.includes(lang);
        const x = toggleX + index * (toggleW + toggleSpacing);
        if (x + toggleW < p.width * 0.95) { // Check overflow slightly more generously
           uiElements.langToggles.push({ x, y: toggleY, w: toggleW, h: toggleH, lang });
           p.stroke(COLORS.letterBorder); p.fill(isSelected ? COLORS.toggleOn : COLORS.toggleOff);
           p.rect(x, toggleY, toggleW, toggleH, 5);
           applyTextStyle(p, { size: FONT_SIZE_UI - 2, color: isSelected ? COLORS.text : '#888', align: p.CENTER, vAlign: p.CENTER });
           p.text(`${flag} ${lang.toUpperCase()}`, x + toggleW / 2, toggleY + toggleH / 2);
        }
    });
    applyTextStyle(p, { size: FONT_SIZE_UI, align: p.LEFT });
    ...
    const diffButtonW = p.constrain(p.width / 5, 80, 110);
    const diffButtonH = 40;
    const diffSpacing = 15;
    ['easy', 'medium', 'hard'].forEach((diff, index) => {
        const isSelected = selectedDifficulty === diff;
        const x = diffButtonX + index * (diffButtonW + diffSpacing);
        if (x + diffButtonW < p.width * 0.95) { // Check overflow
            uiElements.difficultyButtons.push({ x, y: diffButtonY, w: diffButtonW, h: diffButtonH, difficulty: diff });
            p.stroke(COLORS.letterBorder); p.fill(isSelected ? COLORS.buttonHover : COLORS.buttonBg);
            p.rect(x, diffButtonY, diffButtonW, diffButtonH, 5);
            applyTextStyle(p, { size: FONT_SIZE_UI, color: COLORS.buttonText, align: p.CENTER, vAlign: p.CENTER });
            p.text(diff.charAt(0).toUpperCase() + diff.slice(1), x + diffButtonW / 2, diffButtonY + diffButtonH / 2);
        }
    });
    const startBtnW = p.constrain(p.width * 0.4, 120, 200);
    const startBtnH = p.constrain(p.height * 0.08, 40, 60);
    const startBtnX = p.width / 2 - startBtnW / 2;
    const startBtnY = p.height * 0.85 - startBtnH / 2;
    uiElements.startButton = { x: startBtnX, y: startBtnY, w: startBtnW, h: startBtnH };
    let hoverX = dragging.isTouch ? -1000 : p.mouseX; let hoverY = dragging.isTouch ? -1000 : p.mouseY;
    p.fill(COLORS.buttonBg); if (isPointInRect(hoverX, hoverY, startBtnX, startBtnY, startBtnW, startBtnH)) { p.fill(COLORS.buttonHover); }
    p.rect(startBtnX, startBtnY, startBtnW, startBtnH, 10);
    applyTextStyle(p, { size: p.constrain(startBtnH * 0.4, 16, 24), color: COLORS.buttonText, align: p.CENTER, vAlign: p.CENTER });
    p.text('Start Game!', startBtnX + startBtnW / 2, startBtnY + startBtnH / 2);
}

function drawPlayingScreen(p) {
    // (Playing screen drawing logic remains the same as v1.3)
...
function drawGameOverScreen(p) {
    // (Game Over screen drawing logic remains the same as v1.3)
    p.background(BACKGROUND_COLOR);
...
// --- lib/game_logic.js ---
// (startGame, nextWord, checkWinCondition, swapLetters remain the same as v1.3)
function startGame(p) {
    ...
    currentWordPool = shuffleArray([...baseWordPool]);
     if (currentWordPool.length === 0) {
        ...
        alert("Whoops! No words found for that difficulty. Please try another setting.");
        gameState = 'START_SCREEN'; return;
     }
    score = 0; wordsPlayed = -1; gameState = 'PLAYING'; nextWord(p);
}
function nextWord(p) {
    wordsPlayed++;
    if (wordsPlayed >= MAX_WORDS_PER_GAME) { gameState = 'GAME_OVER'; return; }
     if (currentWordPool.length === 0) {
         console.warn("Ran out of unique words!"); gameState = 'GAME_OVER';
         alert("Wow, you played all the words for this difficulty! 🎉 Game Over!"); return;
     }
     currentWordData = currentWordPool.pop();
    wordStates = [];
    selectedLanguages.forEach(lang => {
        const correctWord = currentWordData[lang];
        if (correctWord) {
            const scrambled = scrambleWord(correctWord); // Call the utils function
            wordStates.push({ lang, correctWord, currentLetters: scrambled.split(''), letterBoxes: [], isCorrect: false });
        } else { console.warn(`Word data missing for lang '${lang}' in ${currentWordData.emoji}`); }
    });
    keyboardState.selectedRow = 0; keyboardState.selectedCol = 0;
     if (wordStates.length > 0 && wordStates[0].currentLetters.length > 0) {
         keyboardState.selectedCol = Math.min(keyboardState.selectedCol, wordStates[0].currentLetters.length - 1);
     } else { keyboardState.selectedCol = 0; }
    keyboardState.selectedLetters = [];
    startTime = p.millis(); correctFlashTime = 0;
    console.log(`Starting word ${wordsPlayed + 1}: ${currentWordData.emoji} (${currentWordData.en})`);
}
function checkWinCondition(p) {
    ...
    wordStates.forEach(state => {
        ...
        state.isCorrect = (currentWord.toLowerCase() === state.correctWord.toLowerCase());
        if (!state.isCorrect) allCorrect = false;
    });
    if (allCorrect) {
        console.log("Word Correct!"); correctFlashTime = p.millis();
        let elapsedTime = (p.millis() - startTime) / 1000;
        let points = 0;
        if (elapsedTime <= SCORE_THRESHOLDS.fast) points = SCORE_POINTS.fast;
        else if (elapsedTime <= SCORE_THRESHOLDS.medium) points = SCORE_POINTS.medium;
        else if (elapsedTime <= SCORE_THRESHOLDS.slow) points = SCORE_POINTS.slow;
        else points = SCORE_POINTS.slowest;
        score += points;
        setTimeout(() => {
            if (gameState === 'PLAYING') { // Check state before proceeding
                nextWord(p); keyboardState.selectedRow = 0; keyboardState.selectedCol = 0; keyboardState.selectedLetters = [];
            }
        }, 600);
    }
}
function swapLetters(langIndex, index1, index2) {
    if (langIndex < 0 || langIndex >= wordStates.length) return;
    const state = wordStates[langIndex];
    if (!state || !state.currentLetters) return;
    if (index1 < 0 || index1 >= state.currentLetters.length || index2 < 0 || index2 >= state.currentLetters.length) return;
    console.log(`Swapping in lang ${langIndex}: index ${index1} ('${state.currentLetters[index1]}') with index ${index2} ('${state.currentLetters[index2]}')`);
    [state.currentLetters[index1], state.currentLetters[index2]] = [state.currentLetters[index2], state.currentLetters[index1]];
}
...
// --- Dragging Logic Helper Functions ---
...
const sketch = (p) => {
    p.setup = () => {
        ...
        console.log("Game Setup Complete! v1.3.1");
    };
